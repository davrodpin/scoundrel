...
- Return JSON via `Response.json()` with appropriate HTTP status codes (200,
  201, 204, 400, 401, 404, 422, 500)
...
- Validate all request bodies and query params with Zod schemas at the handler
  boundary
- Return 422 with Zod error details on validation failure
- Define request/response schemas alongside the route file or in a shared
  schemas directory
...
- CSRF protection is mandatory for state-changing methods (POST, PUT, PATCH,
  DELETE)
- Never leak stack traces or internal details in error responses (log them
  instead)

### Logging

- Log errors with LogTape at `error` level using structured data (include route,
  method, status)
- Log unexpected failures; do not log successful requests (leave that to
  middleware)
